    ...
    Private Async Function TryGetMessagesAsync(endpoint As String, pageNo As Integer, pageSize As Integer) As Task(Of List(Of TuyaMessage))
        Try
            ...
            Else
                ...
                Log("")
                Log("💡 DIAGNOSTIC:")
                If errorCode = "28841101" Then
                    Log("   ❌ Erreur 28841101: L'API 'Query Messages' n'est PAS souscrite")
                    Log("")
                    Log("   SOLUTIONS À ESSAYER:")
                    Log("   1️⃣ Vérifiez la VERSION de 'Mobile Push Notification Service'")
                    Log("      → L'API peut être dans Standard/Advanced, pas dans Free/Trial")
                    Log("")
                    Log("   2️⃣ Recherchez un service appelé:")
                    Log("      → 'App Message' ou 'Message Center' ou 'Notification Center'")
                    Log("      → Sur https://iot.tuya.com → Cloud → Products")
                    Log("")
                    Log("   3️⃣ OUVREZ UN TICKET SUPPORT TUYA (solution la plus rapide)")
                    Log("      → https://iot.tuya.com → Support (icône ? en haut)")
                    Log("      → Indiquez: Error 28841101 avec endpoint /v1.0/sdf/notifications/messages")
                    Log("      → Tuya débloquera l'API au niveau de votre compte")
                    Log("")
                    Log("   ⚠️ Ce problème est connu: même avec Mobile Push activé,")
                    Log("      certains comptes n'ont pas accès à Query Messages.")
                    Log("      Un ticket support résout généralement le problème.")
                ElseIf errorCode = "1106" OrElse errorCode = "1100" Then
